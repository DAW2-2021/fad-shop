...
<div class="container">
    ...
    <div class="row">
        <div class="card @if ($petition->state == 'pending') bg-warning
            text-dark
        @elseif ($petition->state == 'accepted') text-white bg-success
        @elseif ($petition->state == 'rejected') text-white bg-danger @endif

            mb-3">
            <div class="col card-header d-flex">
                <div class="w-50 text-capitalize d-flex fw-bold">{{ $petition->user->username }}</div>
                <div class="w-50 text-capitalize d-flex flex-row-reverse fw-bold">{{ $petition->state }}</div>
            </div>
            <div class="card-body">
            ...
            <div class="card-footer d-flex jus">
                @if ($petition->state == 'pending')
                    <button class="btn btn-primary m-2" data-bs-toggle="modal" data-bs-target="#modalModifyPetition" type="submit">Modificar petición</button>
                    ...
                    @elseif ($petition->state == 'rejected')
    ...
    <div class="modal fade" id="modalModifyPetition" tabindex="-1" role="dialog" aria-labelledby="modalModifyPetitionTitle" aria-hidden="true">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                @endif
                <form action="{{ route('petition.admin.update', $petition->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div class="form mb-3">
                        <label class="form-label h4" for="shop_name">Nombre de la Tienda:</label><input class="form-control"
                            placeholder="{{ $petition->shop_name }}" id="shop_name" name="shop_name" type="text" />
                    </div>
                    <div class="mb-3">
